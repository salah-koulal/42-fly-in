...
class Simulator:
    ...
    def _is_finished(self) -> bool:
        for drone in self.drones:
            if drone.current_zone.name.strip() != self.end_zone.name.strip() \
    ...
    def run_turn(self) -> List[str]:
        ...
        link_occupancy = {
            self._get_connection_name(
                c.zone1.name,
                c.zone2.name): 0 for c in self.map_data.connections}
        ...
        idle_drones = [d for d in self.drones if not d.is_traveling and
                       d.current_zone.name.strip(
                       ) != self.end_zone.name.strip() and
                       d.id not in moved_this_turn]

        idle_drones.sort(
            key=lambda d: self.pf.get_distance(
                d.current_zone.name))
        ...
        for drone in idle_drones:
            ...
            for next_name in best_neighbors:
                ...
                conn_obj = next(
                    (c for c in self.map_data.connections
                        if self._get_connection_name(
                            c.zone1.name, c.zone2.name) == conn_name), None)
                # print(f"{conn_obj.zone1.name}<->{conn_obj.zone2.name}")
                ...
                zone_occupancy[next_name] = zone_occupancy.get(
                    next_name, 0) + 1
                link_occupancy[conn_name] = link_occupancy.get(
                    conn_name, 0) + 1
